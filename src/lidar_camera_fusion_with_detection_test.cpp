  ...
  void point_cloud_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
    ...
    std::vector<std::pair<float, float>> x_limits;
    x_limits.emplace_back(-max_depth_, -min_depth_);
    x_limits.emplace_back(min_depth_, max_depth_);
    std::vector<std::pair<float, float>> y_limits;
    y_limits.emplace_back(-max_depth_, -min_depth_);
    y_limits.emplace_back(min_depth_, max_depth_);
    std::vector<std::pair<float, float>> z_limits;
    z_limits.emplace_back(-max_depth_, -min_depth_);
    z_limits.emplace_back(min_depth_, max_depth_);

    // Apply filters to all axes
    filter_point_cloud(cloud, filtered_cloud, "x", x_limits);
    ...
    // // Filtered cloud using improved filtering
    // pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_cloud(new pcl::PointCloud<pcl::PointXYZ>());
    // std::vector<std::pair<float, float>> x_limits{{-max_depth, -min_depth}, {min_depth, max_depth}};
    // std::vector<std::pair<float, float>> y_limits{{-max_depth, -min_depth}, {min_depth, max_depth}};
    // std::vector<std::pair<float, float>> z_limits{{-max_depth, -min_depth}, {min_depth, max_depth}};
