...
LidarCameraFusionNode::LidarCameraFusionNode() 
    ...
    this->declare_parameter<float>("min_value", -10.0);
    this->declare_parameter<float>("max_value", 10.0);
    this->get_parameter("lidar_frame", lidar_frame_);
    this->get_parameter("camera_frame", camera_frame_);
    this->get_parameter("min_value", min_value_);
    this->get_parameter("max_value", max_value_);

    //Log parameters
     RCLCPP_INFO(this->get_logger(), "Loaded Parameters: Camera Frame = %s, MinDepth = %f, MaxDepth = %f",
                camera_frame_.c_str(), min_value_, max_value_);
    ...
    object_point_cloud_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/detected_object_point_cloud", 10);
    ...
    std::vector<size_t> &indices)
{
    ...
    pass.setFilterLimits(min_value_, max_value_);
