...
private:
    ...
    {
        if (!msg) {
            RCLCPP_ERROR(this->get_logger(), "Received null camera info message.");
            return;
        }
    ...
    {
        if (!point_cloud_msg || !image_msg || !detection_msg) {
            RCLCPP_ERROR(this->get_logger(), "Received null message in sync callback.");
            return;
        }
                                                                        ...
                                                                        std::vector<BoundingBox>& bounding_boxes)
    {
        ...
        if (!camera_model_.initialized()) {
            RCLCPP_ERROR(this->get_logger(), "Camera model is not initialized.");
